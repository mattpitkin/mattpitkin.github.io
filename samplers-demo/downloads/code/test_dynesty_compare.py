...
wmask = np.random.rand(len(scaledweights)) < scaledweights

#fig = corner.corner(dpostsamples, labels=[r"$m$", r"$c$"], truths=[m, c], hist_kwargs={'density': True})
fig = corner.corner(res.samples[mask,:], weights=weights[mask], labels=[r"$m$", r"$c$"], truths=[m, c], hist_kwargs={'density': True})
fig = corner.corner(res.samples[wmask,:], fig=fig, color='g', hist_kwargs={'density': True})
